...
% (iterating $\mathit{stmtList_1}$ first).
%\begin{algorithmic}[1]
%\FOR{each statement $\mathit{s}$ in $\mathit{stmtList_1}$}
%\FOR{each statement $\mathit{s'}$ in $\mathit{stmtList_2}$}
%\IF{matched($\mathit{s}$, $\mathit{s'}$)}
%\RETURN $\langle$$\mathit{s}$, $\mathit{s'}$$\rangle$
%\ENDIF
%\ENDFOR
%\ENDFOR
%\RETURN null
...
\IF{matched($\mathit{stmt_{old}}$, $\mathit{stmt_{new}}$)}
