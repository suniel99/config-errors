...
is the number of all possible predicate profiles\todo{Is this the number of
predicates affected by configuration options, or the number of predicate
profiles?  The paper is inconsistent about this.  Does it make a difference
which you choose, and how are they treated?  This needs to be clarified
throughout.} in the program
...
true). Its value ranges from 0 meaning completely different, to 1 meaning
 almost\todo{cut ``almost?''} the same, 
...
\ourtool computes the $Deviation$ value for each predicate $p$\todo{Or
predicate profile?}
appearing in two execution profiles $e$ or\todo{``and''?} $f$, and
