...
Our work is significantly different from the existing approaches.
...
requires a substantial amount of time and effort that a
...
Many automated debugging techniques~\cite{Jones:2002,
roessler-issta-2012, Horwitz:1988,
clause07july} have been developed to localize errors in software.
Program slicing~\cite{Horwitz:1988} and tainting~\cite{clause07july}
are two techniques to determine which statements and inputs
could affect a particular variable value. $\blacksquare$


Delta debugging~\cite{Zeller:2002:ICC} is an algorithm for isolating software error causes.
...
Researchers have designed techniques to improve software configuration
...
However, those automated recommendation approaches have a quite different
focus than our technique. They focus on reducing the burden of
...
iTree~\cite{itrees} improves combinatorial interaction testing
by discovering sets of configurations to test that are smaller
but can achieve higher teesting coverage. However,
those techniques aim to find possible errors in
