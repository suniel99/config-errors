...
that lead to software crash or assertion failures.
...
Second, many existing approaches~\cite{Attariyan:2010:ACT, Whitaker:2004:CDS}
...
the profiles from the undesirable execution with correct executions, and
then inferring the root cause configuration options.

%diagnoses a configuration error from a different perspective.
%It compares the profile from an undesirable execution with 
...
analyzes a large number of execution traces collected from deployed software
to isolate software failure causes. However, CBI
...
links the undesirable behavior to specific configuration options.
%Furthermore, CBI 
...
to automatically learn a good configuration from historical data.
However, their approach aims to reduce the burden of
...
but can achieve higher testing coverage. Compared to \ourtool,
those testing techniques can be used to find new errors in
a configurable software sysem earlier, but can not
