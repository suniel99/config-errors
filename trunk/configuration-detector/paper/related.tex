...
debugging, and configuration-aware software
analysis techniques.
 
%\subsection{Software Configuration Error Diagnosis}
%\vspace{1mm}
\noindent \textbf{\textit{Software Configuration Error Diagnosis.}}
Software configuration error diagnosis is recognized
as an important research problem~\cite{Attariyan:2008:UCD, 
...
First, most previous approaches focus exclusively on configuration errors
that lead to a crash or assertion failure~\cite{Attariyan:2008:UCD, 
Whitaker:2004:CDS, 
...
affected predicate behavior is more general.
...
cannot be directly applied to diagnose a non-crashing error.
...
It reduces differences between a working state and a broken
state to isolate a set of failure-inducing changes.
When using delta debugging, the user must provide 
a single nearby working state and 
a testing oracle to check
whether an intermediate program state behaves correctly --- both of which are difficult tasks.
Furthermore, as a minimization technique, delta debugging
is not applicable to data (i.e., configuration option values)
that are missing nor ones that are incorrect.
By contrast, \ourtool eliminates these assumptions by comparing
...
debugging domain.
%Clarify~\cite{Ha:2007:IER} uses program features
...
The CBI project~\cite{Liblit:2005:SSBI}
analyzes executions collected from deployed software
to isolate software failure causes. Significantly different than
\ourtool, CBI correlates a predicate's evaluation result (either true or
false) rather than its true ratio and execution frequency
with the observed behaviors. In addition,
%Compared to \ourtool, besides
%the significant difference in the error diagnosis algorithm,
CBI identifies likely buggy statements as its final output, while
...
%management~\cite{Garvin:2011} and
understand and test the behavior of a
configurable software system~\cite{Garvin:2011, Qu:2008:CRT}.
...
Compared to \ourtool, those techniques
...
%in the software engineering community. Representative work includes delta debugging~\cite{Zeller:2002:ICC},
...
% LocalWords:  Chronus AutoBash misconfiguration PeerPressure ConfAid CBI Qu
% LocalWords:  ConfAnalyzer reachability Garvin combinatorial
