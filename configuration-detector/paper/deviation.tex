...
is the number of predicates affected by configuration options
...
true). Its value ranges from 0 meaning completely different predicate behavior, to 1 meaning
 the same predicate behavior, 
...
appearing in two execution profiles $e$ and $f$, and
ranks them in decreasing order.  The two execution profiles $e$ and $f$ are for the
same program, so they have exactly the same set of predicates affected by
