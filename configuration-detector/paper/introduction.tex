...
message, ift is often cryptic or misleading~\cite{Yin:2011:ESC, Attariyan:2010:ACT, Hubaux:2012, rangefix}.
...
automated test generation tool~\cite{randoop},
...
In addition, for this scenario, there is no obvious testing oracle\todo{Why
...
ineffective. Moreover, the person who reported the bug had already
minimized the bug report:  if any part of the 
configuration or
input is removed, Randoop either crashes or
...
error while executing a \ourtool-instrumented version of Randoop,
then used \ourtool
to diagnose its root cause by analyzing the recorded trace.
\ourtool produced
...
different between the recorded erroneous execution
...
test generation.
Although \CodeIn{maxsize}'s default value is carefully chosen
...
Finally, this problem isu resolved if
the user changes \CodeIn{maxsize} to a larger value, for example 500.
...
so that it and records the runtime behaviors of affected predicates
in a trace file.
...
\ourtool selects, from a pre-built database, correct traces that are as
similar as possible to the incorrect one.
Then, it identifies the predicates whose dynamic behaviors deviate the most
between correct and incorrect executions.
\ourtool uses\todo{vague; clarify} the obtained deviated predicates to
identify the likely root causes.  Finally, it
outputs a ranked list of suspicious options and explanations.
% (between
...
database, which contains traces
...
found that even a small database of 6--16 traces worked remarkably
...
On average, \ourtool's 5th report was the root cause; in over half of all
...
Assuming the database of correct executions already exists,
\ourtool takes less than $\blacksquare$ minutes on average to diagnose
...
We compared \ourtool to three variants and one\todo{explain why only one}
existing technique. The first variant replaces
thin slicing with traditional slicing~\cite{Horwitz:1988}
to identify affected predicates for each
configuration option. The second and third variant use
statement-level profiling~\cite{Jones:2002} and method-level
invariant detection~\cite{Ernst:1999}
to replace the predicate-level profiling in
\ourtool, respectively.
The existing technique, called ConfAnalyzer~\cite{Rabkin:2011:PPC}, is
based on dynamic information flow analysis to reason
about the root cause of a configuration error.
Our experimental results show that: using thin slicing
provides more accurate diagnosis results than traditional
slicing; and \ourtool significantly outperformed the
other two variants, suggesting
that focusing on the behavior of relevant program predicates can be a
better choice. 
Compared to ConfAnalyzer~\cite{Rabkin:2011:PPC}, 
...
Finally, we investigated the effects of varying the comparison traces
from \ourtool's database.
Our experimental results show that varying the
trace selection strategy can result in substantially different
