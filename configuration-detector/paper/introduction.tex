...
minimized: if any part is removed, Randoop either crashes or
no longer exhibits this error.

In fact, this report does not reveal a real bug
...
error by executing Randoop while \ourtool
attaches to monitor and record the execution flow in a trace file,
and use \ourtool
...
For this configuration error, we show
its relevant code snippet in Figure~\ref{fig:example}.
In Randoop, when a new test (line 100, in the form of
a method-call sequence) is generated,
Randoop first compares its length with \CodeIn{maxsize}'s
value (default value: 100). If the
...
and works remarkably well for many programs (including those used to test
...
Finally, this problem can be easily resolved if
...
and end-users when they encountered a suspected configuration error
...
on the affected predicates to monitor its execution,
and records the runtime behaviors of those affected predicates
in a trace file.
%diagnosed code when erroneous behavior
%is observed.

\item \textbf{Configuration Deviation Analysis}.
When a configuration error occurs, \ourtool uses
the recorded trace to query its pre-built database, selects similar traces
from known correct executions for comparison,
and then identifies the predicates whose dynamic behaviors deviate the most.
\ourtool further uses the obtained deviated predicates to
identify the likely root causes and
...
Besides, our technique also provides explanation on
...
from \subjectnum configurable software.
...
thin slicing with the traditional slicing~\cite{Horwitz:1988}
