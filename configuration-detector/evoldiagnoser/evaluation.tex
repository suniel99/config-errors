...
provide us a configuration error he encountered when using Javalanche.
...
may simply treat the software behaviors as having been validated. Further,
...
ability to identify the behaviorally-changed predicates with
...
\noindent\textbf{\prevtool}, proposd in our previous work~\cite{Zhang:2013:ADS},
is an automated
software configuration error diagnosis technique. %Unlike \ourtool,
\prevtool is \textit{not} cognizant of
software evolution, and it diagnoses configuration errors on
...
correct execution trace, \prevtool only uses the deviation
value to reason about the most deviated predicates, while
ignoring the statements controlled by the predicate's evaluation result. 
...
\noindent \textbf{\conftool}, proposed by Rabkin and
...
(after applying the same configuration from the old version)
...
the program behavior changes, while ignoring the statements
...
choice, and showing that considering the number of controlled statements
can substantially increases the diagnosis accuracy.
...
\ourtool considers both the predicate behavior changes
and the number of affected statements in diagnosing
configuration errors. We next an alternative technique
that only uses the predicate behavior change in erorr
diagnosis, as \prevtool does. This alternative techniques
...
many predicates' behaviors can be changed, but each predicate
...
the predicate behavior changes and the impacted statements,
